...
public class Problem6Tests
{
    ...
    [EmbeddedResource("Advent2022.Tests.Data.problem-6-full-input.txt", "1855")]
    public async Task TestPart1(Stream stream, string expected)
    {
        var solution = await Problem6.SolvePart1Async(stream);
    ...
    [EmbeddedResource("Advent2022.Tests.Data.problem-6-full-input.txt", "3256")]
    public async Task TestPart2(Stream stream, string expected)
    {
        var solution = await Problem6.SolvePart2Async(stream);
        Assert.Equal(expected, solution);
    }
}